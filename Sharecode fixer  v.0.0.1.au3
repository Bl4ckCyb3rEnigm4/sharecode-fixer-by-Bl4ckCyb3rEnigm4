...
while True
	$Data=ClipGet()
	if StringInStr($Data, "http")<>0 And StringInStr($Data, "/")<>0 And StringInStr($Data, @CRLF)<>0 Then
		...
		While True
			...
			EndIf
		WEnd
		ClipPut(StringTrimLeft($Array[1], StringInStr($Array[1], "http")-1) &StringTrimLeft($Array[2], StringInStr($Array[2], "/")-1))
